...
public:
    void loadFromFile()
    {
        ...
        while (fin >> id >> t.type >> cat >> amt >> t.date)
        {
            ...
            try
            {
                t.amount = stof(encryptDecrypt(amt));
            }
            catch (...)
            {
                continue;
    ...
    void sortByAmount()
    {
        ...
        for (auto &[_, t] : transactions)
            v.push_back(t);
        sort(v.begin(), v.end(), [](Transaction a, Transaction b)
    ...
    void deleteTransactionByIndex()
    {
        if (transactions.empty())
            return (void)(cout << "No transactions to delete.\n");
        displayTransactions();
        int id;
        cout << "Enter ID to delete: ";
        cin >> id;
        if (transactions.erase(id))
    ...
    void processPendingQueue()
    {
        ...
        {
            Transaction t = pendingTransactions.front();
    ...
    void showMenu()
    {
        ...
        do
        {
            ...
            else if (choice == 6)
            {
                ...
                addToPendingQueue(getTransactionInput());
            else if (choice == 8)
                processPendingQueue();
            else if (choice == 9 && currentRole == "admin")
